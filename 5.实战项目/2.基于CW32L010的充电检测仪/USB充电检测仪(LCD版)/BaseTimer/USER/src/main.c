...
long long VIvalue_total=0;//功率总数(电量用)
float WHvalue=0;
	...
	while(1)
	{
		...
		{							
			if(lcd_time>=200)
			{
				...
				//电量换算
				WHvalue=(float) VIvalue_total/3600;		
				if(WHvalue >= 100) sprintf(str," %d Wh ",(int) WHvalue);
				else sprintf(str,(WHvalue >= 10)?" %.1f Wh ":" %.2f Wh " ,(float) WHvalue);				
...
//	else t=Ipiancha-I_Buffer;  //减去偏置
	...
	#else
		KT1=3300;
		KT1=KT1*0.0029; //38mA对应的 0.038*0.005*50/3.3=0.00287878
		if(t<KT1)t=0;	 
		t=t/4095*3300/10*4; 
	#endif
	Ivalue=t;
