...
void Get_ADC_Value(void)
{
  ...
  ADC_GetSqr0Result(&IPV1_Buffer[cnt]);  //电流偏执电压
  ADC_GetSqr1Result(&Curr_Buffer[cnt]);	 //电流
  ADC_GetSqr2Result(&Volt_Buffer[cnt]);	 //VCC电压
	ADC_GetSqr3Result(&V12V_Buffer[cnt]);  //1.2V
	ADC_GetSqr4Result(&NTC_Buffer[cnt]);   //NTC温度
